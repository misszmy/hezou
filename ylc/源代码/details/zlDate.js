...
var pickerEvent = {
    Init: function (elemid,month) {
        ...
        html += htmlObj.header;
        html += '<div class="basefix heigo" id="bigCalendar" style="display: block;">';
        html += htmlObj.left;
        html += htmlObj.right;
    ...
    getRight: function () {
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
                ...
                var child_price=commonUtil.getChildPrice((i-c));
                var package_price=commonUtil.getPackagePrice((i-c));
                var stock = commonUtil.getStock((i - c));
                var child_stock=commonUtil.getChildStock((i-c));
                var package_stock=commonUtil.getPackageStock((i-c));
                var stock_id=commonUtil.getStockId((i-c));
                var priceStr = "";
                var classStyle = "";
                if (price != -1) {
				...
				if(obj.year==new Date().getFullYear()&&obj.month==new Date().getMonth()+1&&i-c==new Date().getDate()){
					html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '" child_price="' + child_price + '" stock="' + stock +'" child_stock="' + child_stock +'" package_price="' + package_price +'" package_stock="' + package_stock +'" stock_id="' + stock_id +'"><a><span class="date basefix">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
				else{
                	html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '" child_price="' + child_price + '" stock="' + stock +'" child_stock="' + child_stock +'" package_price="' + package_price +'" package_stock="' + package_stock +'" stock_id="' + stock_id +'"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
                if (index == 6) {
...
var commonUtil = {
    getStockId: function (day) {
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].date == dt) {
                return obj.priceArr[i].id.split('.')[0];
            }
        }
        return -1;
    },
    ...
    getChildPrice: function (day) {
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].date == dt) {
                return obj.priceArr[i].child_price.split('.')[0];
            }
        }
        return -1;
    },
    getPackagePrice: function (day) {
        var dt = obj.year + "-";
        if (obj.month < 10)
        {
            dt += "0"+obj.month;
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].date == dt) {
                return obj.priceArr[i].package_price.split('.')[0];
            }
        }
        return -1;
    },
    getStock: function (day) {
        var dt = obj.year + "-";
        if (obj.month < 10)
        {
            dt += "0"+obj.month;
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].date == dt) {
                return obj.priceArr[i].stock.split('.')[0];
            }
        }
        return -1;
    },
    getChildStock: function (day) {
        var dt = obj.year + "-";
        if (obj.month < 10)
        {
            dt += "0"+obj.month;
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].date == dt) {
                return obj.priceArr[i].child_stock.split('.')[0];
            }
        }
        return -1;
    },
    getPackageStock: function (day) {
        var dt = obj.year + "-";
        if (obj.month < 10)
        {
            dt += "0"+obj.month;
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].date == dt) {
                return obj.priceArr[i].package_stock.split('.')[0];
            }
        }
        return -1;
    },
    chooseClick: function (sender) {
        ...
        var child_price = sender.getAttribute("child_price");
        var package_price = sender.getAttribute("package_price");
        var stock = sender.getAttribute("stock");
        var child_stock = sender.getAttribute("child_stock");
        var package_stock = sender.getAttribute("package_stock");
        var stock_id = sender.getAttribute("stock_id");
        var el = document.getElementById(elemId);
        if (el != null) {
            el.value = date;
            $("#adult_price").text(price);
            $("#child_price").text(child_price);
            $("#package_price").text(package_price);
            $("#adult_stock").text(stock);
            $("#child_stock").text(child_stock);
            $("#package_stock").text(package_stock);
            $("#package_stock").text(package_stock);
            $("input[name='stock_id']").val(stock_id);
